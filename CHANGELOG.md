...
- **`presonus-studiolive-api` 1.9.1** adds `getSwitch()` / `setSwitch()`. These parameters were readable through the state tree but had no setter, so writing one previously meant reaching into the library's private `_sendPacket`. Paths were verified against a StudioLive 16R, including the nested ones such as `line/chN/limit/limiteron`.
